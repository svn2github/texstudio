...
TitledPanel::TitledPanel(QWidget *parent) :
	QFrame(parent), mToggleViewAction(0), closeAction(0), pageSelectActions(0), vLayout(0),
	topbar(0), collapsedTopbar(0), stack(0)
{
	...
	closeAction->setIcon(QIcon(":/images/test/closebutton.png"));
...
// TODO
void TitledPanel::setHiddenWidgets(const QStringList& hiddenIds) {
	hiddenWidgetsIds=hiddenIds;
}

// TODO
QStringList TitledPanel::hiddenWidgets() const {
	return hiddenWidgetsIds;
}
...
void TitledPanel::updateTopbar() {
	...
	if (visiblePageCount == 1) {
		//lbTopbar->setText(firstVisiblePage->m_title);
		//topbar->addWidget(lbTopbar);
	} else if (visiblePageCount > 1) {
		QComboBox *cbTopbarSelector = new QComboBox(topbar);
